...
/**
 * Simulador de temperatura de un espacio
 */
public class SimuladorTemperatura {
	...
	/**
	 * Simula la temperatura en un espacio
	 * 
	 * @param espacio
	 *            espacio que simular
	 */
	public static void simular(Espacio espacio) {
		...
		if ((hour >= SimuladorEspacio.HORA_ENTRADA)
			...
			if (!tempObj.igualTemperaturaQue(new Temperatura(
					TEMPERATURA_ABIERTO))) {
		...
		if (temp.mayorTemperaturaQue(tempObj)) {
			if (random.nextInt(100) < PROBABILIDAD_AULA_LAB) {
				espacio.cambiarTemperatura(new Temperatura(temp
						.getTemperature() - 0.5));
			}
